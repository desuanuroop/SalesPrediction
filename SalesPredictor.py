...
# Load data
data = pd.read_csv('E:/Education/1st MS/Sem 1/DM/train.csv', parse_dates = ['Date'])
store = pd.read_csv('E:/Education/1st MS/Sem 1/DM/store.csv') 
...
#Validation of model created.
for i in range(5,20, 5):
	rf_validation = RandomForestRegressor(n_jobs = -1, n_estimators = i)
	rf_validation.fit(X_validation, Y_validation)
	out_of_bag_prediction_validation = rf_validation.oob_prediction_
	generalization_error  = your_error_metric(out_of_bag_prediction_validation, validation).
	if i == 5:
		prv_generalization_error = generalization_error
		prv_model = i
	else:
		if generalization_error > prv_generalization_error:
			print('previous model is better than present')
			break;
rf = RandomForestRegressor(n_jobs = -1, n_estimators = prv_model)
rf.fit(X_train, y_train)

# Load and process test data
test = pd.read_csv('E:/Education/1st MS/Sem 1/DM/test.csv', parse_dates = ['Date'])
